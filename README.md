...
- Starts Apple's CoreDevice system-audio RTP stream (the same path Xcode Mirror uses) alongside video. Linux decode of that bitstream is still experimental; video continues if audio is silent.
...
- Audio is the phone's system output only. There is no microphone capture and no audio sent to the phone. Audio start or decode failure leaves video running.
